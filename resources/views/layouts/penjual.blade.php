...
<head>
    ...
    <style>
        ...
        .sidebar { width: 230px; min-height: 100vh; background: #0f3460; position: fixed; top: 0; left: 0; z-index: 1050; display: flex; flex-direction: column; padding: 24px 16px; transition: transform .25s ease; }
...
<div class="sidebar" id="sidebar">
    ...
    <nav class="nav flex-column gap-1 flex-grow-1">
        ...
        <a href="{{ route('penjual.ebook.index') }}" class="nav-link {{ request()->routeIs('penjual.ebook.*') ? 'active' : '' }}">
            <i class="bi bi-book"></i> Ebook Saya
        </a>
        <a href="{{ route('penjual.ebook.create') }}" class="nav-link">
            <i class="bi bi-plus-circle"></i> Tambah Ebook
        </a>
        <a href="{{ route('penjual.penjualan') }}" class="nav-link {{ request()->routeIs('penjual.penjualan') ? 'active' : '' }}">
            <i class="bi bi-cart-check"></i> Penjualan
        </a>
        <a href="{{ route('penjual.laporan') }}" class="nav-link {{ request()->routeIs('penjual.laporan') ? 'active' : '' }}">
